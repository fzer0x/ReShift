...
EXPORTS.scanMemory = function(pattern, moduleName) {
    Logger.info(`Scanning memory for pattern: ${pattern}` + (moduleName ? ` in ${moduleName}` : ''));
    const results = [];
        ...
        ranges.forEach((range, idx) => {
            try {
                ...
                if (rangeMatches && rangeMatches.length > 0) {
                    Logger.info(`  Range ${idx}: Found ${rangeMatches.length} matches in ${moduleName || range.toString()}`);
                ...
                Logger.warn(`Scan error in range: ${rangeErr.message}`);
            }
        });

        Logger.success(`Found ${results.length} total matches`);
        return results;
    } catch (e) {
